...
var keysApi = require('../redisApi/keysApi');
...
function getKeys (req, res, next) {
  ...
  getConnection(req.redisConnections, connectionId, function (err, redisConnection) {
    ...
    var limit = req.params.limit || req.app.configuration.maxKeysToQuery;
    console.log(sf('loading keys by prefix "{0}"', prefix));
    
    keysApi.getAll(prefix, limit, redisConnection, next, function(keys){      
      if (keys.length > 1) {
...
function getKeysTree (req, res, next) {
  ...
  getConnection(req.redisConnections, connectionId, function (err, redisConnection) {
    keysApi.getAll(search, req.app.configuration.maxKeysToQuery, redisConnection, next, function(keys){
...
function postKeys (req, res, next) {
  ...
  getConnection(req.redisConnections, connectionId, function (err, redisConnection) {
    if (req.query.action === 'delete') {
      deleteKeys(key, redisConnection, req, res, next);
    } else {
...
function deleteKeys (keyQuery, redisConnection, req, res, next) {
  keysApi.getAll(keyQuery, req.app.configuration.maxKeysToQuery, redisConnection, next, function(keys){
